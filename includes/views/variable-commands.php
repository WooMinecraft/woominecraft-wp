<div class="woo_minecraft">
	...
	<div class="form-fields">
		...
		<span class="woo_minecraft_copyme" style="display:none">
			<input type="text" name="minecraft_woo[general][variable][<?php echo $loop; ?>][]" value="" class="short" placeholder="<?php _e( 'Use %s for player name', 'wmc' ); ?>"/>
			<input type="button" class="button button-small delete remove_row" value="Delete">
		</span>
		<?php if ( ! empty( $meta ) ) : ?>
			<?php foreach ( $meta as $command ) : ?>
				<span>
					<input type="text" name="minecraft_woo[general][variable][<?php echo $loop; ?>][]" value="<?php echo $command; ?>" class="short"/>
