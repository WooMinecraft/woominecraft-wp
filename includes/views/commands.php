...
<div class="woo_minecraft">
	...
	<table class="woominecraft commands" cellpadding="5px">
		...
		<tbody>
			...
			<tr class="row">
				<td><input type="text" name="wmc_commands[<?php echo $command_key; ?>][0][command]" class="widefat" placeholder="<?php _x( 'give %s apple 1', 'Sample Command', 'woominecraft' ); ?>" /> </td>
				<td>
					<select name="wmc_commands[<?php echo $command_key; ?>][0][server]" >
					<?php
			...
			<?php else : ?>
				<?php $offset = 0; foreach ( $commands as $server_key => $command ) : ?>
					...
					<tr class="row">
						<td><input type="text" name="wmc_commands[<?php echo $command_key; ?>][<?php echo $offset; ?>][command]" class="widefat" placeholder="<?php _e( 'give %s apple 1', 'woominecraft' ); ?>" value="<?php echo $player_command; ?>" /> </td>
						<td>
							<select name="wmc_commands[<?php echo $command_key; ?>][<?php echo $offset; ?>][server]" >
								<?php
								foreach ( $servers as $server ) {
